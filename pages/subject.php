...
$subjects = $db->getFull('subject', ' AND id > 0' . $search_where . ' ORDER BY name ASC LIMIT 36');
?>
...
<main class="min-h-screen bg-slate-50">
  ...
  <nav aria-label="Breadcrumb" class="mx-auto max-w-7xl px-4 mt-4 pt-3">
    <ol class="flex flex-wrap items-center gap-2 text-sm text-slate-500 pb-3">
      ...
      <li>/</li>
      <li class="font-semibold text-slate-900" aria-current="page">Subject</li>
    </ol>
  </nav>
    ...
    <div class="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <div class="max-w-md mx-auto">
          <div class="flex">
            <input
              ...
              placeholder="বিষয়সমূহ অনুসন্ধান করুন"
      ...
      <div id="subjectResults">
        ...
        <div class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <a href="<?php echo domain; ?>subject/<?php echo htmlspecialchars($subject['slug']); ?>" class="group">
        ...
        <div class="flex flex-col items-center justify-center py-12">
          <h3 class="text-base font-medium text-slate-700">কোনো বিষয় পাওয়া যায়নি</h3>
        </div>
        <?php endif; ?>
      </div>
...
(function(){
  ...
  function searchSubjects(q){
    $.post('ajax.php',{action:'search_subjects',search:q},function(html){
      $('#subjectResults').html(html);
    });
  }
  $('#subjectSearch').on('input',function(){
    var self=this;
    clearTimeout(debounceTimer);
    debounceTimer=setTimeout(function(){searchSubjects($.trim($(self).val()))},300);
  });
  $('#subjectSearchBtn').on('click',function(){
    searchSubjects($.trim($('#subjectSearch').val()));
  });
})();
</script>
