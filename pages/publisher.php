...
$publishers = $db->getFull('publisher', ' AND id > 0' . $search_where . ' ORDER BY name ASC LIMIT 36');
?>
...
<main class="min-h-screen bg-slate-50">
  ...
  <nav aria-label="Breadcrumb" class="mx-auto max-w-7xl px-4 mt-4 pt-3">
    <ol class="flex flex-wrap items-center gap-2 text-sm text-slate-500 pb-3">
      ...
      <li>/</li>
      <li class="font-semibold text-slate-900" aria-current="page">Publisher</li>
    </ol>
  </nav>
  ...
  <div class="mx-auto max-w-7xl px-4 pb-12">
    <div class="bg-white rounded-xl border border-slate-200 p-6">
        ...
        <div class="max-w-md mx-auto">
          <div class="flex">
            <input
              ...
              placeholder="প্রকাশনীসমূহ অনুসন্ধান করুন"
      ...
      <!-- Publishers List -->
      <div id="publisherResults">
        <?php if (!empty($publishers)): ?>
        <div class="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
          <?php foreach ($publishers as $publisher): ?>
          <a href="<?php echo domain; ?>publisher/<?php echo htmlspecialchars($publisher['slug']); ?>" class="group">
        ...
        <div class="flex flex-col items-center justify-center py-12">
          <h3 class="text-base font-medium text-slate-700">কোনো প্রকাশনী পাওয়া যায়নি</h3>
        </div>
        <?php endif; ?>
      </div>
...
(function(){
  ...
  function searchPublishers(q){
    $.post('ajax.php',{action:'search_publishers',search:q},function(html){
      $('#publisherResults').html(html);
    });
  }
  $('#publisherSearch').on('input',function(){
    var self=this;
    clearTimeout(debounceTimer);
    debounceTimer=setTimeout(function(){searchPublishers($.trim($(self).val()))},300);
  });
  $('#publisherSearchBtn').on('click',function(){
    searchPublishers($.trim($('#publisherSearch').val()));
  });
})();
</script>
